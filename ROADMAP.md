...
| ID | Mô tả | Ưu tiên | Độ khó | Thời gian | File ảnh hưởng | App | Trạng thái |
|---|---|---|---|---|---|---|---|
| **P1-01** | Xoá email/mật khẩu/UID thật bị hardcode trong script dev; thay bằng giá trị placeholder hoặc đọc từ biến môi trường (`--dart-define`) | Critical | Thấp | 0.5 ngày | `lib/dev/create_test_user.dart`, `lib/dev/demo_seeder.dart` | Mobile | ✅ Done (2026-07-05) — đã gộp thêm `lib/dev/seed_firestore.dart` (phát hiện cùng lỗi, không có trong danh sách gốc). Cả 3 file dùng `String.fromEnvironment(...)` với placeholder mặc định, không còn UID/email/password thật trong source. |
